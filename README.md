...
- `OPENCODE_CLAUDE_STRICT_MCP` — `1` passes `strictMcpConfig` and `ENABLE_CLAUDEAI_MCP_SERVERS=false`, so Claude sees only the bridged OpenCode tools; servers from `.mcp.json`, `~/.claude.json` and claude.ai connectors are not loaded (OpenCode's own MCP servers stay reachable through its tools). Off by default
...
- `OPENCODE_CLAUDE_SPILL_CHARS` — tool results longer than this are cut to a head/tail with a note on how much of the middle was dropped; the model re-runs the command narrowed if it needs the middle. Nothing is written to disk (default `8000`; `0` keeps the old history truncation and inlines live results). File reads (`read`) are never cut on the turn they run; in transferred history a long read becomes a pointer to its source file
- `OPENCODE_CLAUDE_OUTPUT_STORE_MB` — memory cap for cut outputs (default `16`; `0` disables). A cut output is kept in process memory and the note gives Claude an id for the local `output_slice` tool, which returns up to 8000 chars per call. Least recently used outputs are evicted at the cap, an output above 4 MB is never kept, and an output idle for 30 minutes is dropped. `output_slice` is answered inside the plugin, so OpenCode does not show these calls; with `OPENCODE_CLAUDE_DEBUG=1` each call is logged
