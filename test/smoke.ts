...
async function main() {
  ...
  // ---- Rate-limit tracker + tool/plan behavior (mocked Agent SDK) ----
  {
    ...
    try {
      ...
      // tools/list is what Claude sees: same order, output_slice included.
      const listedTools = await (
        seenParams as any
      ).mcpServers.opencode.instance.server._requestHandlers.get("tools/list")(
        { method: "tools/list", params: {} },
        {},
      );
      assert.deepEqual(
        listedTools.tools.map((t: { name: string }) => t.name),
        ["output_slice", "question", "todowrite"],
      );
      const sliceTool = listedTools.tools[0];
      assert.deepEqual(sliceTool.inputSchema.required, ["id"]);
      assert.deepEqual(sliceTool._meta, { "anthropic/alwaysLoad": true });
